...
router.post("/", withUser, upload.single('image'), async (request, response, next) => {
    ...
    try {
        ...
        if (user) {
            user.myRecipes.push(savedRecipe._id);
            await user.save();
        }
...
router.delete("/:id", withUser, async (request, response, next) => {
    try {
        ...
        const user = await User.findById(userId);
        if (user) {
            user.myRecipes = user.myRecipes.filter(id => id.toString() !== request.params.id);
            await user.save();
        }
...
router.put("/:id", withUser, async (request, response, next) => {
    ...
    try {
        ...
        const updatedRecipe = await RecipeModel.findByIdAndUpdate(request.params.id, { name, category, area, instructions, image, tags, youtube, source, ingredients }, { new: true, runValidators: true, context: 'query' });
