...
router.post("/favorites/:recipeId", withUser, async (request, response, next) => {
    try {
...
router.delete("/favorites/:recipeId", withUser, async (request, response, next) => {
    try {
        const { recipeId } = request.params;
        const userId = request.userId;
        const user = await User.findById(userId);
...
router.get("/favorites", withUser, async (request, response, next) => {
    try {
        const userId = request.userId;
...
router.get("/recipes", withUser, async (request, response, next) => {
    try {
        const userId = request.userId;
